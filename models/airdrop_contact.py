...
class MassAirdropContactListRel(models.Model):
    """ Intermediate model between mass airdrop list and mass airdrop contact
        Indicates if a contact is opted out for a particular list
    """
    _name = 'airdrop.contact.subscription'
    _description = 'Airdrop Subscription Information'
    _table = 'airdrop_contact_list_rel'
    _rec_name = 'contact_id'

    contact_id = fields.Many2one('airdrop.contact', string='Contact', ondelete='cascade', required=True)
    list_id = fields.Many2one('airdrop.list', string='Airdrop List', ondelete='cascade', required=True)
    ...
    _sql_constraints = [
        ('unique_contact_list', 'unique (contact_id, list_id)',
         'A airdrop contact cannot subscribe to the same airdrop list multiple times.')
    ]
    ...
    def write(self, vals):
        ...
        if vals.get('unsubscription_date'):
            vals['opt_out'] = True
        return super(MassAirdropContactListRel, self).write(vals)


class MassAirdropContact(models.Model):
    """Model of a contact. This model is different from the partner model
    because it holds only some basic information: name, email. The purpose is to
    be able to deal with large contact list to email without bloating the partner
    base."""
    _name = 'airdrop.contact'
    _inherit = ['mail.thread.blacklist']
    _description = 'Airdrop Contact'
    _order = 'email'
    _airdrop_enabled = True

    def default_get(self, fields):
        """ When coming from a airdrop list we may have a default_list_ids context
        ...
        res = super(MassAirdropContact, self).default_get(fields)
    ...
    list_ids = fields.Many2many(
        'airdrop.list', 'airdrop_contact_list_rel',
        'contact_id', 'list_id', string='Airdrop Lists')
    subscription_list_ids = fields.One2many('airdrop.contact.subscription', 'contact_id', string='Subscription Information')
    country_id = fields.Many2one('res.country', string='Country')
    tag_ids = fields.Many2many('res.partner.category', string='Tags')
    opt_out = fields.Boolean('Opt Out', compute='_compute_opt_out', search='_search_opt_out',
                             help='Opt out flag for a specific airdrop list.'
                                  'This field should not be used in a view without a unique and active airdrop list context.')
    ...
    def _search_opt_out(self, operator, value):
        ...
        if 'default_list_ids' in self._context and isinstance(self._context['default_list_ids'], (list, tuple)) and len(self._context['default_list_ids']) == 1:
            ...
            contacts = self.env['airdrop.contact.subscription'].search([('list_id', '=', active_list_id)])
    ...
    def create(self, vals_list):
        ...
        return super(MassAirdropContact, self.with_context(default_list_ids=False)).create(vals_list)

    @api.returns('self', lambda value: value.id)
    def copy(self, default=None):
        """ Cleans the default_list_ids while duplicating airdrop contact in context of
        a airdrop list because we already have subscription lists copied over for newly
    ...
    def action_add_to_airdrop_list(self):
        ctx = dict(self.env.context, default_contact_ids=self.ids)
        action = self.env["ir.actions.actions"]._for_xml_id("mass_airdrop.airdrop_contact_to_list_action")
